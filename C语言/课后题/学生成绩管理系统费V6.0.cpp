...
A:printf("***********student achievement management system v5.0*********\n");
		...
		printf("* 12.save these information to a file.                       *\n");
		printf("* 13.read these information.                                 *\n");
		printf("* 0.exit.                                                    *\n");
		printf("**************************************************************\n");
...
void save(struct student* a)
{
	...
	for(int i = 0;i < student;i ++)
	{
		fprintf(fp, "%-9ld%-7s%", a[i].number, a[i].name);
		for(int k = 0; k < score_m; k++)
			fprintf(fp, "%.1f  ", a[i].score[k]);
		fprintf(fp, "%-8.1f%-7.1f\n", a[i].total,a[i].average);
	}
	fclose(fp);
}
...
void read()
{
	...
	for(int i = 0;i < student;i ++)
	{
		fscanf(fp, "%ld%s", &a[i].number, &a[i].name);
		for(int k = 0; k < score_m; k++)
			fscanf(fp, "%f  ", &a[i].score[k]);
		fscanf(fp, "%f%f\n", &a[i].total,&a[i].average);
	}
	printf("\n");
	for(int i = 0; i < 3; i++)
	{
		printf("%-9ld%-7s%", a[i].number, a[i].name);
		for(int k = 0; k < score_m; k++)
			printf("%.1f  ", a[i].score[k]);
		printf("%-8.1f%-7.1f\n", a[i].total,a[i].average);
	}
	fclose(fp);
}
